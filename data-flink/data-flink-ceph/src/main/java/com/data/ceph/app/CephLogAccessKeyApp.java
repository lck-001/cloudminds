...
public class CephLogAccessKeyApp {
        ...
        SingleOutputStreamOperator<CephAccessLog> cephAccesslogDS = cephNginxLogDS.map(new MapFunction<CephNginxLog, CephAccessLog>() {
            ...
            public CephAccessLog map(CephNginxLog cephNginxLog) throws Exception {
                ...
                if (request_header_info != null) {
                    if (request_header_info.authorization != null) {
                        ...
                        if (s.find()) {
                            ...
                            //去除access_key中不正确格式
                            if (!cephAccessLog.access_key.matches("^[a-z0-9A-Z]+$")){
                                cephAccessLog.access_key = cephAccessLog.access_key.split("(?![0-9A-Z])")[0];
                            }
                        }
                    }
                ...
                if (cephNginxLog.fields.request.split("/").length >= 3) {
                    ...
                    //格式化bucket
                    if (cephAccessLog.bucket.contains("?") || cephAccessLog.bucket.contains(" ")){
                        cephAccessLog.bucket = cephAccessLog.bucket.split("[(?| )]")[0];
                    }
...
//                            ctx.timerService().registerProcessingTimeTimer(timerState.value());
        ...
        // 将record-->转换成jsonstring-->写入到hdfs
        allDataDS.map(line->JSON.toJSONString(line)).addSink(fileSink);
