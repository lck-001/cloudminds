...
import com.data.http.bean.TransientSink;
    ...
    public static <T> SinkFunction getJdbcSink(String sql) {
        SinkFunction<T> sink = JdbcSink.<T>sink(
                ...
                new JdbcConnectionOptions.JdbcConnectionOptionsBuilder()
                        .withUrl(HttpConf.CLICKHOUSE_URL)
                        .withDriverName(HttpConf.CLICKHOUSE_DRIVER)
                        .withUsername(HttpConf.CLICKHOUSE_USERNAME)
                        .withPassword(HttpConf.CLICKHOUSE_PASSWORD)
                        .build());
        return sink;
    }
}