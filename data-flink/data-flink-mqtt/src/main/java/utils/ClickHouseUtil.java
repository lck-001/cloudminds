    ...
    public static <T> SinkFunction getJdbcSink(String sql) {
        SinkFunction<T> sink = JdbcSink.<T>sink(
                ...
                new JdbcConnectionOptions.JdbcConnectionOptionsBuilder()
                        .withUrl(MqttConfig.CLICKHOUSE_URL)
                        .withDriverName(MqttConfig.CLICKHOUSE_DRIVER)
                        .withUsername(MqttConfig.CLICKHOUSE_USERNAME)
                        .withPassword(MqttConfig.CLICKHOUSE_PASSWORD)
                        .build());
        return sink;
    }
}