...
-- boss新整合的数据
with roc_t_dict as (
    select t1.robot_type_id,
           t1.robot_type_name,
           t2.robot_type_inner_name
    from (
             select label   as robot_type_name,
             ...
             full join (
        select label   as robot_type_inner_name,
               `value` as robot_type_id
        from cdmods.ods_roc_db_c0002_t_dict_s_d
        where dt = '${e_dt_var}'
          and TYPE = 'robot-type-hari'
        group by label, `value`
...
--                            when length(t1.update_time) >= 23 then nvl(from_utc_timestamp(SUBSTRING(REGEXP_REPLACE(t1.update_time,'T',' '),0,23),'PRC'),'')
                  ...
                  where dt >= date_sub('${s_dt_var}',1) and dt <= '${e_dt_var}'
              ) x1
                  left join (
             select
                 robot_type_id,
