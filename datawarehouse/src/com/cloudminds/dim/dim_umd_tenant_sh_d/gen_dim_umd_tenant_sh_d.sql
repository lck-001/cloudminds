...
union_roc_boss as (
    select
        ...
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.vop_id,'') != '',t1.vop_id,nvl(t2.vop_id,''))
             else coalesce(t1.vop_id,t2.vop_id,'')
        end as vop_id,
        ...
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.country,'') != '',t1.country,nvl(t2.country,''))
             else coalesce(t1.country,t2.country,'')
        end as country,
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.province,'') != '',t1.province,nvl(t2.province,''))
             else coalesce(t1.province,t2.province,'')
        end as province,
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.city,'') != '',t1.city,nvl(t2.city,''))
             else coalesce(t1.city,t2.city,'')
        end as city,
        ...
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.phone_area_code,'') != '',t1.phone_area_code,nvl(t2.phone_area_code,''))
             else coalesce(t1.phone_area_code,t2.phone_area_code,'')
        ...
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.project_name,'') != '',t1.project_name,nvl(t2.project_name,''))
             else coalesce(t1.project_name,t2.project_name,'')
        end as project_name,
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.district,'') != '',t1.district,nvl(t2.district,''))
             else coalesce(t1.district,t2.district,'')
        end as district,
        ...
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.boss_status,-99999998) != -99999998,t1.boss_status,nvl(t2.boss_status,-99999998))
             else coalesce(t1.boss_status,t2.boss_status,-99999998)
        end as boss_status,
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.boss_status_name,'未知') != '未知',t1.boss_status_name,nvl(t2.boss_status_name,'未知'))
             else coalesce(t1.boss_status_name,t2.boss_status_name,'未知')
        end as boss_status_name,
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.roc_status,-99999998) != -99999998,t1.roc_status,nvl(t2.roc_status,-99999998))
             else coalesce(t1.roc_status,t2.roc_status,-99999998)
        end as roc_status,
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.roc_status_name,'未知') != '未知',t1.roc_status_name,nvl(t2.roc_status_name,'未知'))
             else coalesce(t1.roc_status_name,t2.roc_status_name,'未知')
        ...
        case when t1.tenant_id is not null and t2.tenant_id is not null then if(nvl(t1.service_address,'') != '',t1.service_address,nvl(t2.service_address,''))
             else coalesce(t1.service_address,t2.service_address,'')
