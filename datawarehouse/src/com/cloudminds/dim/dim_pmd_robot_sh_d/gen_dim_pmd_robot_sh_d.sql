 ...
 roc_t_robot as (
     ...
     from (
          select
              ...
              regexp_replace(nvl(trim(robot_name),''), '\r|\n|\t', '') as robot_name,
...
column_dim as (
    select
        ...
        collect_set(case when (robot_manufacturer_id is not null and robot_manufacturer_id != '') or (robot_manufacturer_name is not null and robot_manufacturer_name != '') then concat_ws('->',robot_id,end_time,concat_ws('###',robot_manufacturer_id,robot_manufacturer_name)) else null end ) as robot_manufacturer_dim,
