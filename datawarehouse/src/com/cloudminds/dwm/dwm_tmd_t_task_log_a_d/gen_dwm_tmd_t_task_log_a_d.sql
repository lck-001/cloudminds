...
--           tenant dim
        ...
        nvl(t6.industry_name,'') as tenant__industry_name,
...
--          robot dim
        ...
        nvl(t1.robot_name,'') as robot__robot_name,
        nvl(t3.robot_type_id,'')  as robot__robot_type_id,
        nvl(t1.robot_type,'') as robot__robot_type_inner_name,
...
--          rcu dim
        t1.rcu_id as rcu__rcu_id,
        nvl(t5.rcu_name,'') as rcu__rcu_name,
        nvl(t5.imei,'') as rcu__imei,
        nvl(t5.did,'') as rcu__did,
        nvl(t5.did_credential_id,'') as rcu__did_credential_id,
        nvl(t5.rcu_manufacturer_name,'') as rcu__rcu_manufacturer_name,
        nvl(t5.model,'') as rcu__model,
        nvl(t5.os,'') as rcu__os,
        nvl(t5.version_code,'') as rcu__version_code,
        nvl(t5.status,-99999998) as rcu__status,
        nvl(t5.status_name,'未知') as rcu__status_name,
        nvl(t5.cpu,'') as rcu__cpu,
        nvl(t5.ram,'') as rcu__ram,
        nvl(t5.rom_capacity,'') as rcu__rom_capacity,
        nvl(t5.rom_available_capacity,'') as rcu__rom_available_capacity,
        nvl(t5.sd_capacity,'') as rcu__sd_capacity,
        nvl(t5.sd_available_capacity,'') as rcu__sd_available_capacity,
        nvl(t5.camera,'') as rcu__camera,
        nvl(t5.wifi_mac,'') as rcu__wifi_mac,
        nvl(t5.bluetooth_mac,'') as rcu__bluetooth_mac,
        nvl(t5.regist_time,'') as rcu__regist_time,
        nvl(t5.activate_time,'') as rcu__activate_time,
        nvl(t5.deactivate_time,'') as rcu__deactivate_time,
        nvl(t5.description,'') as rcu__description,
        nvl(t5.create_time,'') as rcu__create_time,
        --task dim
        t1.task_id as task_task_id,
        nvl(t1.task_name,'') as task_task_name,
        nvl(t7.map_id,'') as task_map_id,
        nvl(t7.map_name,'') as task_map_name,
        nvl(t7.task_type_id,-99999998) as task_task_type_id,
        nvl(t7.task_type_name,'') as task_task_type_name,
        nvl(t7.work_types,'') as task_work_types,
        nvl(t7.work_type_names,'') as task_work_type_names,
        nvl(t7.schedule_type,'') as task_schedule_type,
        nvl(t7.schedule_start_time,'') as task_schedule_start_time,
        nvl(t7.schedule_end_time,'') as task_schedule_end_time,
        nvl(t7.schedule_days,'') as task_schedule_days,
        nvl(t7.repeat_times,-99999998) as task_repeat_times,
        nvl(t7.repeat_type,-99999998) as task_repeat_type,
        nvl(t7.repeat_type_name,'') as task_repeat_type_name,
        nvl(t7.sub_task,'') as task_sub_task,
        nvl(t7.is_del,-99999998) as task_is_del,
        nvl(t7.create_time,'') as task_create_time,
        nvl(t7.update_time,'') as task_update_time
    from  cdmdwd.dwd_crio_tmd_t_task_log_a_d t1
             left join (
        select
            tenant_id,
            tenant_name,
        ...
        select
            ...
            create_time ,
            update_time
        from cdmdim.dim_pmd_rcu_sh_d
        where dt = date_sub(current_date,1)
    ) t5 on t1.rcu_id = t5.rcu_id
             left join cdmdim.dim_umd_industry_a_d t6 on t2.industry_id = t6.industry_id left join cdmdim.dim_tmd_task_sh_d t7 on t1.task_id=t7.task_id
    where ( t2.start_time is  null  or t1.event_time between t2.start_time and t2.end_time) and
        ...
        ( t5.start_time is null  or t1.event_time between t5.start_time and t5.end_time) and
        ( t7.start_time is null  or t1.event_time between t7.start_time and t7.end_time)
)

insert overwrite table cdmdwm.dwm_tmd_t_task_log_a_d
