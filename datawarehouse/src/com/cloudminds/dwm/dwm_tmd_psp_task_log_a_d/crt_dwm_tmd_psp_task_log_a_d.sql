-- cdmdwm.dwm_tmd_psp_task_log_a_d 巡逻机器人任务日志
create external table cdmdwm.dwm_tmd_psp_task_log_a_d (
    ...
    task_exec_start_time string comment '本次任务开始时间',
    task_plan_end_time string comment '本次任务计划停止时间',
    plan_repeat_times int comment '本次任务计划圈数',
    plan_point_tasks int comment '本次任务计划巡更点数量',
    start_event_time string comment '巡逻事件时间,这里用的入库时间进行填充的',
    is_finish int comment '0进行中 1结束了',
    task_exec_stop_time string comment '本次任务停止时间',
    ...
    stop_event_time string comment '巡逻事件时间,这里用的入库时间进行填充的',
...
stored as parquet
location '/data/cdmdwm/dwm_tmd_psp_task_log_a_d';