...
--           tenant dim
        ...
        nvl(t2.tenant_type_name,'') as tenant__tenant_type_name,
...
--          robot dim
        ...
        nvl(t1.robot_type_inner_name,'') as robot__robot_type_inner_name,
        ...
        t1.dt
    from (select * from cdmdwd.dwd_harix_vmd_robot_detect_image_info_i_d where dt>=date_sub('${s_dt_var}',1) and dt<='${e_dt_var}') t1
             left join (
        select
            tenant_id,
            tenant_name,
    ...
    ) t2 on t1.tenant_id = t2.tenant_id and t1.k8s_env_name=t2.k8s_env_name
             left join (
        select
            robot_id,
            robot_name,
        ...
        where dt=date_sub(current_date,1)
    ) t3 on t1.robot_id = t3.robot_id and t1.k8s_env_name=t3.k8s_env_name
             left join (
        select
            customer_id,
            customer_name,
        ...
        where dt=date_sub(current_date,1)
    ) t4 on t2.customer_id = t4.customer_id and t1.k8s_env_name=t4.k8s_env_name
             left join cdmdim.dim_umd_industry_a_d t5 on t2.industry_id = t5.industry_id
    where (t1.event_time between t2.start_time and t2.end_time) and
        ( t1.event_time between t3.start_time and t3.end_time) and
        ( t1.event_time between t4.start_time and t4.end_time)
)

insert overwrite table cdmdwm.dwm_vmd_robot_detect_image_info_i_d partition(dt)
