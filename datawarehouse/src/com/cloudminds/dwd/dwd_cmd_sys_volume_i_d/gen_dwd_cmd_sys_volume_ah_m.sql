...
roc as (select
  regexp_extract(json_extract,'"serviceCode":"([^"]+?)"',1) as service_code,
  regexp_extract(json_extract,'"robotId":"([^"]+?)"',1) as robot_id,
  regexp_extract(json_extract,'"accountId":"([^"]+?)"',1) as robot_account_id,
  regexp_extract(json_extract,'"robotType":"([^"]+?)"',1) as robot_type,
  regexp_extract(json_extract,'"tenantId":"([^"]+?)"',1) as tenant_id,
  regexp_extract(json_extract,'"speakVolume":"([^"]+?)"',1) as sys_volume,
  regexp_extract(json_extract,'"timestamp":"([^"]+?)"',1) as event_time,
  k8s_env_name
