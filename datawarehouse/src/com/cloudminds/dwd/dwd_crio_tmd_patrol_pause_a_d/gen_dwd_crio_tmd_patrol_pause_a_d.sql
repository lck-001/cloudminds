...
with patrol_log as (
    ...
    ) t where event_type in ('pause','resume')
),
pause_table as (
    select
        oid,
        ...
        event_time
    from patrol_log
    lateral view json_tuple(description,'taskId', 'taskName', 'projectCode','projectName','runningTime','finishTimes','mileage','finishPointTasks','battery') a
    as programme_id,programme_name,project_code_desc,project_name_desc,running_time,finish_times,mileage,finish_point_tasks,current_battery
)
insert overwrite table cdmdwd.dwd_crio_tmd_patrol_pause_a_d
    select
        oid,
        event_id,
        robot_id,
