...
crss_vending_log_tmp as (
  select
    ...
    unix_timestamp(t1.create_time)-unix_timestamp(t2.create_time) as diff_time
  from (
    select
        *
    ...
    and nvl(ext_data,'')!='') t1
    left join crss.dim_vending t2 on t1.vending_id=t2.id
...
crss_vending_log_deal as (
  ...
  from(
    ...
    from (
        select
          * ,
          row_number() over (partition by guid order by diff_time asc) as rank
        from crss_vending_log_tmp) a
    where rank=1) t1
...
event_union as (
  ...
  from (
      ...
      where (dt=date_sub('${s_dt_var}',1) or dt=date_add('${e_dt_var}',1))) t
)
