...
insert overwrite table cdmdwd.dwd_cms_svo_user_qa_i_d partition(dt) select
distinct
    nvl(id, '') as user_qa_id ,
    nvl(uuid,'') as uuid ,
    cast(sys_cate_id as int ) as sys_cate_id,
    sv_cate_id,
    nvl(cate_id, '') as user_qa_cate_id,
    nvl(agent_id, '') as agent_id,
    nvl(agent_secret, '') as agent_secret,
    nvl(title,'') as title,
    ...
    nvl(svmsg, '') as sv_msg,
    nvl(news_id, '') as news_id,
    nvl(news_url, '') as news_url,
    nvl(payload, '') as  payload ,
