...
with patrol_log as (
    ...
    ) t where event_type='onlineTime'
),
online_time_table as (
    select
        oid,
        ...
        nvl(online_time,0.00) as online_time,
        pub_time,
        event_time
    from patrol_log
    lateral view json_tuple(description,'taskId', 'taskName', 'projectCode','projectName','onlineTime') a
    as programme_id,programme_name,project_code_desc,project_name_desc,online_time
)
insert overwrite table cdmdwd.dwd_crio_tmd_patrol_online_time_a_d
    select
        oid,
        event_id,
        robot_id,
