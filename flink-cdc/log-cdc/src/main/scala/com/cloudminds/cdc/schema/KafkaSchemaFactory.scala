...
class KafkaSchemaFactory(model: KafkaSourceModel) extends SchemaFactory {
  override def createSchema: util.HashMap[String, String] = {
    ...
    val kafkaSchema = new util.HashMap[String, String]()
    for (kv <- tableSchema.entrySet){
      ...
      kafkaSchema.put(kv.getKey,schema.toString)
    }
    kafkaSchema
  }
}
