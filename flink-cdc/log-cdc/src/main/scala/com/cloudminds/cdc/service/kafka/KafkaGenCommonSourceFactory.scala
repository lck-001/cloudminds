...
class KafkaGenCommonSourceFactory(commonProp: CommonProp) extends GenCommonSourceFactory {
  val logger: Logger = LoggerFactory.getLogger(classOf[KafkaGenCommonSourceFactory])
  ...
  override def getCommonSource(str: String): CommonSource = {
    try {
      ...
      if (b) {
        ...
        assert(null != tableRule && tableRule.nonEmpty, "no kafka rule define, please set kafka rule ")
        Breaks.breakable {
          for (rule <- tableRule) {
